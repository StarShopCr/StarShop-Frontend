...
(async () => {
  ...
  if (!cli) {
    console.error(
      '❌ Need `stellar` or `soroban` CLI in PATH for bindings generation.'
    );
    process.exit(1);
  }
  ...
  const subcmd =
    cli === 'stellar' ? 'contract bindings typescript' : 'contract bindings ts';
  const genCmd = [
    `${cli} ${subcmd}`,
    `--rpc-url "${cfg.rpc}"`,
  ...
  rootPkg.dependencies[`@contracts/${name}`] =
    `file:./packages/contracts/${name}`;
  ...
  if (wantWallet && !canCreateWallet) {
    console.warn(
      '⚠️ Wallet creation skipped: `stellar` CLI not found (required for `stellar keys`).'
    );
  }
  ...
  console.log(
    `   Import:   import { ContractClient } from "@contracts/${name}";\n`
  );
})();
